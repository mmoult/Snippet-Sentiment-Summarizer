...
public class ReviewCleaner extends DataCleaner {
	...
	static {
		...
		for(int i=0; i<FileResources.types.length; i++) {
			inFiles[i] = FileResources.fileRoot + "review_" + FileResources.types[i] + ".json";
		}
	}
	
	public static void main(String args[]) throws IOException {
		new ReviewCleaner().cleanAll(true);
	}
	...
	public void cleanAll(boolean sentenceDivision) throws IOException {
		...
		for(int i=0; i<inFiles.length; i++) {
			System.out.println("Processing file " + (i+1) + "/"+inFiles.length + " (" +
					FileResources.reviewFiles[i] +")");
			try {
				cleanFile(inFiles[i], FileResources.reviewFiles[i], sentenceDivision);
			}catch(IOException e) {
